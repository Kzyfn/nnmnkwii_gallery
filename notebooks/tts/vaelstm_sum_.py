...
def parse():
    ...
    parser.add_argument(
        ...
        type=int,
        #required=True,
        default=1,#'data/yokohama/yokohama_v8.4.xlsx',
    )
    parser.add_argument(
        '-zd',
        '--z_dim',
...
class VAE(nn.Module):
    ...
    def reconstruction(self, z, linguistic_features, mora_index):
        z_tmp = torch.tensor([[0]*z_dim]*linguistic_features.size()[0], dtype=torch.float32, requires_grad=True).to(device)
        count = 0
        prev_index = 0
        for i, mora_i in enumerate(mora_index):
        ...
        return self.fc4(h3)#torch.sigmoid(self.fc3(h3))
